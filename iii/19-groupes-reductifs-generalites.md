...
First,[^N.D.E-XIX-4] `U = rad_u(G) ∩ Centr_G(Q)` is an invariant unipotent subgroup of `Centr_G(Q)`; let us show that it
is smooth and connected. If we make `Q` operate on `rad_u(G)` by inner automorphisms, `U` is none other than the scheme
of invariants of this operation. But this scheme is smooth and connected, by Lemma 1.4 below.

Consequently, `U` is a closed subgroup of `rad_u(Centr_G(Q))`. On the other hand, by *Bible*, Exp. 12, § 3, Cor. to Th.
1, one has `rad_u(Centr_G(Q))(k) ⊂ rad_u(G)(k)`. The equality (1.3.1) follows.
...
`Centr_G(Q)` is representable by a closed subscheme of `G`, of finite presentation over `G`, by Exp. XI 6.11
...
`S`; and since moreover `ε_Q` is of finite presentation, `H^Q` is locally of finite presentation over `S`.) This proves
(i).
...
sufficient that `G` be unipotent (i.e. that `G = rad_u(G)`), by *Bible*, § 6.4, Cor. 1 to Th. 4 or [Ch05], § 6.5, Cor. 1
to Th. 5.
...
`Centr(G)` (i.e. the neutral component `Centr(G)⁰`) is a smooth connected solvable subgroup, invariant in `G`, hence
contained in `rad(G)`; conversely, `rad(G)` is an invariant torus in `G`, hence central (*Bible*, § 4.3, Cor. to Prop.
2), whence (iv).

<!-- original page 5 -->

**Remark 1.6.3.** *If `G` is reductive and if `dim(G) ≠ rgred(G)`, then `dim(G) − rgred(G) ⩾ 2`. Indeed, this difference
is always even (cf. 1.10 below).*
...
the union of the `f(T) f(rad_u(G))` is dense in `G′`; but `f(T)` consists of semisimple elements, so `f(T) = {e}`, `G′`
being unipotent; this shows that `f(rad_u(G))` is dense in `G′`. Therefore, by *Bible*, § 5.4, Lemma 4 or [Ch05], § 6.1,
Lemma 1,[^N.D.E-XIX-8] `f(rad_u(G))` is an open subgroup of `G′`; the latter being connected, it follows that
...
`G / rad(G)` is semisimple (*Bible*, § 9.4, Prop. 2), and `G / rad_u(G)` is reductive. One calls the *semisimple rank of
...
`T` (via `T → G` and the adjoint operation of `G`) as
...
`dim(G) − dim(T)` of them (*Bible*, § 13.4, Cor. 2 to Th. 3). Moreover, if `α` is a root, so is `−α` (*Bible*, § 12.2,
Cor. to Prop. 1). (As usual, one writes the group structure of `Hom_{k-gr.}(T, G_{m,k})` indifferently additively or
multiplicatively.) It follows that, for `G` reductive,
...
(1.6.2 (i)), non-solvable (definition of a singular torus), hence `dim(Centr_G(Q)) − rgred(G) ⩾ 2`, which proves that
`G = Centr_G(Q)` and `Q` is central in `G`.
...
then `G` is smooth over `S`, by EGA IV₄, 17.5.2.
...
torus of `G`. The set of `s ∈ S` such that `G_s` is a reductive `s`-group, of semisimple rank `1` and with maximal torus
`T_s`, is an open subset `U` of `S`.*
...
is lower semicontinuous. Let `U₂` be the set of points of `S` where this function is `> 1`; it is open. By 1.11, the set
of `s` such that `G_s` is reductive, of semisimple rank `1`, with maximal torus `T_s`, is `U = U₁ ∩ U₂`; moreover, for
every `s ∈ U`, `W_G(T)_s` has exactly two points.
...
*the sheaf of sections of this vector fibration. Suppose moreover that `G` is smooth over `S`; then `ω¹_{G/S}` and hence
`Lie(G/S)` are locally free `O_S`-modules of finite type, and one has (cf. I 4.6.5.1):*
...
*Proof.*[^N.D.E-XIX-21] Denote by `π : G → S` the structure morphism and `ε : S → G` the unit section. Since `G` is flat
over `S` at `ε(s₀)` and `G_{s̄₀}` reduced (hence smooth over `s̄₀`, cf. VI_A 1.3.1), `G` is smooth over `S` at the point
`ε(s₀)` (EGA IV₄, 17.5.1), i.e. there exists an open neighborhood `V` of `ε(s₀)` such that `π|V` is smooth. Then,
`S′ = ε⁻¹(V)` is an open subset of `S`, and `G_{S′}` is smooth over `S′` at the points of `ε(S′)`. Since `G` has
connected fibers, `G_{S′}` is smooth over `S′`, by VI_B, 3.10. So, replacing `S` by `S′`, one may assume `G` smooth over
`S`.
...
which represents the functor of subtori of rank `r₀` of `G` (IX 1.4). Smoothness entails that `𝓜_{r₀}` admits a rational
point over a finite separable extension of `κ(s₀)` (cf. EGA IV₄, 17.15.10 (iii)). Thus there exists `S′ → S` étale
equipped with a point `s′₀` mapping to `s₀` such that `G_{s′₀}` admits a torus of rank `r₀`. Therefore, replacing `S` by
`S′`, one may assume that `G_{s₀}` admits a torus of rank `r₀`.
...
there exists an étale morphism `f : S″ → S′` splitting `T` and such that `f⁻¹(s′₀) ≠ ∅`. Since an étale morphism is open
and the assertions of (i) are local for the étale topology, one may therefore assume that `G` possesses a split torus
`T`,[^N.D.E-XIX-23] maximal at `s₀`. Write therefore `T = D_S(M)` and let
...
One has therefore proved (i) and the first assertion of (ii); it remains to prove that the Weyl group `W_{G_U}(T_U)` is
finite over `U`, i.e. "that it has the same number of points in each geometric fiber" (cf. SGA 1, I 10.9 and EGA IV₃,
...
**Corollary 2.6.** *Let `G` be an `S`-group affine and smooth over `S`, with connected fibers. The set of points `s ∈ S`
such that `G_s̄` is reductive (resp. semisimple) is an open subset `U` of `S`, and the functions*
...
`G_{U_i}` comes by base change from a reductive group over a noetherian ring (in fact, a finitely generated algebra over
`Z`). Likewise, if `G` possesses a split maximal torus `T`, one may further assume that `T_{U_i}` comes from a split
maximal torus of the preceding group, ….*
...
I, § 4.7). For every character `α` of `T` (i.e. `α ∈ Hom_{S-gr.}(T, G_{m,S})`), one defines a subfunctor of `W(F)` by
...
[^N.D.E-XIX-29] It is clear that (ii) ⇒ (i), and one sees easily that (iii) ⇒ (i). One has (i) ⇒ (ii), for if `s′ ∈ S′`
lies over `s` and if `α_{s′}` is the unit character, then so is `α_s`. Finally, to prove (i) ⇒ (iii), one reduces to the
case where `T` is diagonalizable and concludes by Exp. VIII 3.2 a).

**3.4.** [^N.D.E-XIX-30] Let `G` be a reductive `S`-group scheme, `T` a maximal torus of `G`. Let `α` be a root of `G`
with respect to `T`. Then, by 2.5.0 and 1.12, `g^α` is a locally free `O_S`-module of rank one. Moreover, by 1.10, `−α`
is also a root of `G` with respect to `T`. In particular, if `G` is of semisimple rank `1`, one has by 1.11 and 1.12:
...
`S`-scheme defined by `R`; thanks to condition (ii), one sees easily that this morphism is an open and closed immersion
whose image is none other than `⋃_{α ∈ R} α(S)` (each `α ∈ R` being considered as a section `S → 𝓜`).

Let `𝓡` be the functor of roots of `G` with respect to `T`: by definition, `𝓡(S′)` is the set of roots of `G_{S′}` with
respect to `T_{S′}` for every `S′ → S`; if `S′ = ∅`, one sets `𝓡(∅) = {e}`, and if `S′ ≠ ∅` then the inclusion
...
subsheaf of `𝓜` for (fpqc). Locally for this topology, `G` possesses a root system with respect to `T` (take for example
`T` split and use the proof of Th. 2.5). By Exp. IV 4.6.8 and the theory of descent of open (resp. closed)
subschemes,[^N.D.E-XIX-31] one obtains:

**Proposition 3.8.** *Let `S` be a scheme, `G` an `S`-group, `T` a maximal torus of `G`. The functor `𝓡` of roots of `G`
with respect to `T` is representable by a twisted constant finite `S`-scheme (Exp. X 5.1) which is an open and closed
subscheme of `Hom_{S-gr.}(T, G_{m,S})`.*
...
`T` (i.e. a section of `𝓡`). Consider the kernel `Ker(α)` of `α`, its unique maximal torus `T_α`, and the centralizer of
the latter, `Z_α = Centr_G(T_α)`. It is an `S`-group closed in `G`, reductive (2.8) of semisimple rank `1` (1.12).
Moreover,
...
**3.10.** Let `S` be a scheme, `G` a reductive `S`-group, `T` a maximal torus of `G`, `α` a root of `G` with respect to
`T`. If `q ∈ Q` and if `q α` is a root of `G` with respect to `T`, then `q = 1` or `q = −1`. This follows at once from
1.12.
...
*Proof.* Indeed, `V = V(F)` for some quasi-coherent `O_S`-module `F`. Since `V` is smooth over `S`, then `F ≃ ω¹_{V/S}`
...
Then `W(L)×` is a principal homogeneous bundle (locally trivial) under `G_{m,S}`. One denotes `Γ(S, L)× = W(L)×(S)`.*
...
*This correspondence is effected by `f ↦ W(f) ↦ W(f)(1)`. It is compatible with extension of the base. One may therefore
consider `W(L)×` as the "scheme of trivializations of `W(L)`".*
...
**Definition 4.6.** *Let `S` be a scheme, `G` an `S`-group scheme, `T` a subgroup of `G`, `α` a character of `T`, `L` an
`O_S`-module. Let*
...
inner automorphisms. In other words, for every `S′ → S` and all `t ∈ T(S′)` and `x ∈ W(L)(S′) = Γ(S′, L ⊗ O_{S′})`, one
has*
...
**Reminder 4.10.** *Let `k` be an algebraically closed field, `G` a reductive `k`-group, `T` a maximal torus of `G`, `α`
a root of `G` with respect to `T`. There exists a monomorphism*
...
once from the elementary fact: the gcd of the binomial coefficients `(q choose i)`, for `i ≠ 0, q`, is `p` if `q = p^n`,
`p` prime, and `1` in the contrary case.) The morphism defined by the `q`-th power
...
For each `n ⩾ 0`, let `S_n` be the set of `s ∈ S` such that `(α′ ∘ f)_s = n α_s`. One knows (Exp. IX 5.3) that the `S_n`
are open and closed, and that `(α′ ∘ f)_{S_n} = n α_{S_n}`. Moreover, since `α_{s₀} ≠ 0`, one may, shrinking `S` if
necessary, assume that `α` is non-zero on each fiber (same reference), which entails that the `S_n` are disjoint.
Shrinking `S` if necessary, one may therefore assume that one is in one of the two following cases: there exists an `n`
such that `S = S_n`, or all the `S_n` are empty.

Let `m ⩾ 0` be such that `S_m = ∅`; I claim that then `a_m = 0`; indeed `α′ ∘ f` and `m α` are distinct on each fiber of
`S`, and one has:

**Lemma 4.13.** *Let `S` be a scheme, `T` an `S`-torus, `α` and `α′` two characters of `T` distinct on each fiber; there
exists a family `{S_i → S}` covering for (fpqc), and for each `i` a `t_i ∈ T(S_i)`, such that `α(t_i) − α′(t_i) = 1`.*
...
every "point" of `U ∩ G`. Conversely, it must be shown that if `u ∈ U(S′)` (for every `S′ → S`), and if `u` verifies the
six conditions of the statement, then `det(u) = 1` and `u` also verifies (1′), (4), (4′) and (5).
...
other hand, one has `G_s = (G₁)_s`; but it is known that the group of automorphisms of the Lie algebra of `PGL_2` over a
field of characteristic `0` is `PGL_2` itself, which is connected and semisimple.
...
that they have the same geometric points. For the fibers at points `s ≠ s₀`, this follows from the fact that
...
of `G₀ (= G_k)` (one exists, for example because the scheme of maximal tori of `G₀` is smooth over `k`, Exp. XII, 7.1
c)); since `k` is separably closed, `T₀` is split (cf. X 1.4) and is therefore given by a monomorphism of groups
...
`_n G = Hom_{S-gr.}((Z/nZ)_S, G) = Ker(G --n--> G)`). But `𝓟` is smooth over `S` (Exp. IX 3.6), hence `u₀ ∈ 𝓟(k)` lifts
to a section `u ∈ 𝓟(S)` (Hensel's lemma, Exp. XI 1.11):
...
Let then `H⁰` be the neutral component of `H`; it is a subgroup scheme of `G`, smooth and with connected fibers, whose
special fiber is a torus. By Exp. X 8.1, it is a torus, necessarily split (Exp. X 4.6). Put `H⁰ = T` and let
...
`G`. These are smooth by Exp. XI 2.4 and 2.4 bis, which proves (i). Assertions (ii) and (iii) are then proved as in Exp.
XI 5.9 and 5.10, whose proof uses in fact only (i) and not the fine theorems Exp. XI 4.1 and 4.2.
...
[^N.D.E-XIX-1]: *N.D.E.*: See the bibliographical references at the end of this Exposé. In particular, a re-edition of
    the *Séminaire Chevalley 1956–58*, cited [Bible], revised by P. Cartier, was published in 2005, cf.
    [Ch05].
...
[^N.D.E-XIX-28]: *N.D.E.*: The original added the hypothesis that `G` be of finite presentation over `S`, which does not
    seem to be used in what follows. In any case, `G` being smooth over `S` and with connected fibers, it
    is quasi-compact and separated over `S` (VI_B, 5.5), hence of finite presentation over `S`.

[^N.D.E-XIX-29]: *N.D.E.*: The original has been simplified; it invoked Exp. IX, 5.2. Let us note, however, that *loc.
    cit.*, 5.3 shows that if `S` is connected, then the conditions of the lemma are verified as soon as (i)
    is verified at one point `s` of `S`.
...
[^N.D.E-XIX-32]: *N.D.E.*: We have kept the original proof; one can also detail it as follows. Let `F` be a
    ...
    and therefore `Lie(V/S) = (ω¹_{V/S})ⱽ ≃ Fⱽ`. If one supposes `V` smooth over `S` then, by (1), `F` is locally free
    ...
    Now, if `U` is an `S`-scheme equipped with a section `τ : S → U`, and if one is given an isomorphism `φ : V ⥲ U` of
    "pointed" `S`-schemes, i.e. such that `φ ∘ ε = τ` (for example if `U` is an `S`-group), then `φ` induces an
    isomorphism `dφ : Fⱽ ⥲ Lie(U/S)`, whence an isomorphism `ψ : W(Fⱽ) ⥲ W(Lie(U/S))` making the appropriate diagram
    commute, which permits one to identify `U` with `W(Lie(U/S))`. Since the functor `W` is fully faithful, there exists
    a unique isomorphism `exp : W(Lie(U/S)) ⥲ U` such that `Lie(ψ⁻¹ ∘ exp) = id`. In fact, one can see directly that
    `Lie(ψ) = id`, so that `exp = ψ`.
...
[^N.D.E-XIX-37]: *N.D.E.*: "consider the semi-direct product `T ·_α W(L)`, resp. `T′ ·_{α′} W(L′)`" has been removed. On
    the other hand, the hypothesis in (b) that `g` be a morphism of groups, combined with (⋆), amounts to
    saying that the morphism `(t, x) ↦ (f(t), g(x))` is a morphism of groups from `T ·_α W(L)` to
    `T′ ·_{α′} W(L′)`.
...
[^N.D.E-XIX-35]: *N.D.E.*: Here, one has written `W(L)` (with `W` in boldface) since, for an arbitrary `O_S`-module `L`,
    `W(L)` is not necessarily representable. But in what follows, `L` will be assumed locally free of
