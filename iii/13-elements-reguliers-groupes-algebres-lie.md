...
subsheaf of `G ×_S W` inverse image of `v` by `ϕ` is isomorphic to the subsheaf `M_v` of `G` given by `M_v(S′) =` set
of `g ∈ G(S′)` such that `v_{S′} ∈ g · W(S′)`, i.e. such that `g⁻¹ v_{S′} ∈ W(S′)`. If `v` is a section of `W` and not
...
closed immersion, that `X` is representable (it is obtained by descent of the closed subprescheme `Z` of `G ×_S V`
by the faithfully flat quasi-compact morphism `G ×_S V → G/N ×_S V`). Hence diagram `(D)` is a diagram of morphisms of
preschemes over `S`.

We henceforth assume that `S` is the spectrum of a field `k`, and that `G`, `V`, `W` are of finite type over `k`. Let
`𝔫` be the Lie algebra of `N`, so that `dim N ⩽ rank 𝔫`, with equality if and only if `N` is smooth over `k`
(Exp VI [^N.D.E-XIII-1]). Let `a ∈ W(k)`, and consider the subscheme `M_a` of `G` defined above, containing `N`, and
isomorphic to `ϕ⁻¹(a)`; we shall denote by `𝔪_a` its Zariski tangent space at the identity element `e` of `N`, so that
one has
...
surjective, then `N → M_a` is smooth at `e`, hence (being an immersion) is an isomorphism at `e`, which shows that
(i) implies (iii). To prove the equivalence of (ii) and (iii), consider as above `X_a = ψ⁻¹(a)` and use the isomorphism
...
morphism deduced from the preceding by the base change `Spec(𝒪_{M_a, e}) → Spec(𝒪_{X_a, ā})` is an isomorphism;
now this deduced morphism is none other than the morphism `Spec(𝒪_{N, e}) → Spec(𝒪_{M_a, e})`, which proves the
...
[^N.D.E-XIII-1]: Not having identified this reference, we refer to Theorem II.5.2.1 of the book: M. Demazure & P. Gabriel, *Groupes algébriques* I, Masson (1970).
...
denotes the subspace of invariants under the action of `H` deduced from the adjoint representation). When this
condition is satisfied, `N` is smooth and `dim X = dim G`. In any case, `dim X ⩽ dim G`, and this inequality is an
equality if and only if `H` is of finite index in `N`.*
...
**Theorem 2.1.** *Let `G` be a smooth connected algebraic group over the algebraically closed field `k`, `H` a
connected smooth algebraic subgroup, `N` its normalizer, `𝔤`, `𝔥`, `𝔫` the Lie algebras, `X = G ×^N H` the scheme
(fibered over `G/N` with typical fiber `H`) introduced in section 1, `ψ : X → G` the canonical morphism (whose image
is also the image of `ϕ : G × H → G` defined by `ϕ(g, h) = int(g)h = ghg⁻¹`). The following conditions are all
equivalent:*
...
*Proof.* By 2.0, one has `dim X ⩽ dim G`, with equality if and only if `dim H = dim N`, i.e. `H` of finite index in
`N`. From the inequality `dim X ⩽ dim G` it follows that `ψ` is dominant if and only if it is dominant and generically
quasi-finite, or again if and only if `ψ` is generically quasi-finite and `dim X = dim G`. As this latter equality
means also, by 2.0, that `H` is of finite index in `N`, we have proved the equivalence of (vi), (v), (iv bis). The
equivalence of (iv) and (iv bis) is immediate.
...
(II, 5.2.3 (ii)). Hence as `H ⊃ C`, so that `𝔥 ⊃ 𝔠`, it follows that `𝔠 ⊂ 𝔤^T`, which is equivalent (by I, 4.7.3) to
the relation
...
`𝔥 ⊃ 𝔠`; I claim that `H` contains the centralizer `C` of `T` (which will establish (i) ⇔ (ii)). This follows from
the:

**Lemma 2.1.1.** *Let `G` be a smooth algebraic group over the field `k`, `T` a subgroup of multiplicative type of
`G`, `C` its connected centralizer (equal to the centralizer of `T` if `G` is connected and `T` is a torus,
(XII 6.6 b))), `H` a smooth subgroup of `G` containing `T`. In order that `H` contain `C`, it is necessary and
sufficient that its Lie algebra `𝔥` contain that `𝔠` of `C`.*

<!-- label: III.XIII.2.1.1 -->

Indeed, one knows (XI 2.4) that `Centr_G(T)` is smooth over `k`, hence `C` is smooth over `k`; similarly `Centr_H(T)`
is smooth over `k`, and `Centr_H(T) = Centr_G(T) ∩ H` has `𝔥 ∩ 𝔠` as Lie algebra; hence the hypothesis implies that
the smooth subgroup `Centr_H(T)` of the smooth group `Centr_G(T)` has the same Lie algebra, so it contains the
connected component `C` of the latter, and thus `H` contains `C`. *QED*.
...
and we know that `𝔫` is the inverse image of the first member of the preceding relation under the canonical
homomorphism `𝔤 → 𝔤/𝔥` (II 5.2.3 (i)), so that the preceding relation means, by 2.0, that `H = N⁰`, and a fortiori
`H` is of finite index in its normalizer. Now consider the diagram of subgroups
...
correspondence with the set of points of `N(T)/N(T) ∩ N(H)` with values in `k`; but as `H ⊃ C`, one has
`N(T) ∩ H ⊃ C`, hence the preceding set is a quotient of `(N(T)/C)(k)`, which is a finite set, hence is finite. This
proves (i) ⇒ (iii). Conversely, suppose (iii), i.e. `N(T)/N(T) ∩ N(H)` finite and `N(H)/H` finite. Using again the
...
induced by the preceding diagram is bijective on points with values in `k` (in fact, it is an isomorphism), hence as
the latter is finite, so is the former, so `N(T) ∩ H` is of finite index in `N(T)`, hence contains `C = N(T)⁰`, hence
...
map is written (using the usual identifications of the tangent spaces at points of `G` and `H` with the Lie algebras
of `G` and `H`)
...
`(e, a)`, it follows that `ψ` is smooth at `q(e, a) = (ē, a)`, hence étale at this point by reasons of dimension. So
we have proved (i) ⇔ (iii) ⇒ (ii); on the other hand (ii) ⇒ (i), because the smoothness of `N` implies that of `q`.
*QED*.
...
(this is Borel's "density theorem"), it follows that one can find a conjugate of `C` which meets `U`, so one can
assume `U ∩ C ≠ ∅`, i.e. that there exists a non-empty open `V` in `C` such that for every `x ∈ V(k)`, `x` is
contained in a conjugate of `H`. Write `C` as a product
...
non-empty open of `T`), i.e. `t · C_u ∩ V ≠ ∅`, and such that `t` "generates" `T`. Since every algebraic subgroup of
`G` containing a product `t · u` (`t ∈ T(k)`, `u ∈ C_u(k)`) contains the two factors (*Bible* 4 th. 3), it follows,
with the preceding choice of `t`, <!-- original page 260 --> and taking `t · u ∈ V(k)`, that there exists a conjugate
of `H` containing `t`, hence `T`. So we may already assume that one has
...
index in its normalizer in `C`. By *Bible* 7, lemma of section 1, it follows that `C₀ = C`, hence `C = H`, which
proves (vi) ⇒ (i) when `G` is affine.
...
2°) `dim Z ∩ H = 0`: then the canonical morphism `H → G/Z` is a finite morphism, and as `G/Z` is affine by virtue of
XII 6.1, it follows that `H` is affine, hence every homomorphism from `H` into an abelian variety is zero (and even
...
according to which `G` is an extension of an abelian variety `A` by a smooth affine group. Then the image of `H` in
`A` is zero, `H` being affine; on the other hand it is identical to `A`, because the union of its conjugates in `A`
must be dense. So `A = 0`, hence `G` is affine, and we are reduced to the case already treated. This completes the
proof of 2.1.
...
*b) Let `T` be a maximal torus of `G` contained in `H` (which exists by form 2.1 (ii)), and let `C` be the
corresponding Cartan subgroup of `G`. Then `C ⊂ H`. On the other hand, `Norm_G(T)` is a smooth subgroup of `G` and
...
*c) Let `U` be the largest open of `G` such that <!-- original page 262 --> `ψ : X → G` induces a morphism
`ψ⁻¹(U) → U` that is finite and étale. Then `U` is a dense open, and for `g ∈ G(k)`, one has `g ∈ U(k)` if and only if
there exist exactly `d` conjugates of `H` containing `g`, or again if and only if there exist at least `d` distinct
conjugates `H_i` of `H` containing `g` such that for every `i`, `(𝔤/𝔥_i)^{ad(g)} = 0` (where `𝔥_i = Lie(H_i)`).*
...
connected (XII 6.6 b)), hence are equal, which proves that `C ⊂ H`. Moreover, one knows that the normalizer of `T` in
a smooth group containing it is smooth (XI, 2.4 bis), so `Norm_G(T)` and `Norm_{Norm_G(H)}(T)` are smooth (N.B. we have
noted that `N = Norm_G(H)` is smooth at the end of the statement of 2.1); also `Norm_N(T)` contains `C`, which is of
finite index in `N(T)`, so it is of finite index in `N(T)`. Using the theorem of conjugacy for maximal tori in `H`,
one sees that the index in question equals the number of conjugates of `H` containing `T`, or what amounts to the same,
...
completes the proof of b). One will note that we have in fact shown that the set of conjugates of `H` containing `T`
is a homogeneous set <!-- original page 263 --> under the group of rational points of
...
birational morphism `ψ : X → G`, given that `G` is normal. The equivalence of these conditions with (iv) follows at
once from the last assertion of 2.1 characterizing the set of points of `X` at which `ψ` is étale.
...
*Proof.* Since condition 2.1 (i) is satisfied, one can apply 2.4 (iii) ⇒ (i), which establishes a). In *Bible* 7 (in
the case where `G` is affine) the points of `U(k)` are called the *regular points* of `G(k)`, and we shall follow this
terminology, calling `U` the *open of regular points* of `G`. (N.B. The proof given in the *Bible* <!-- original page 265 -->
that the set in question is itself open is incorrect, but we have obtained it in the present Exposé under more general
conditions.)
...
When `g ∈ G(k)` is an element contained in a Cartan subgroup (for example a regular element), which we may assume to
be `C`, then by 2.5 (iv) one sees that `(𝔤/𝔠)^{ad(g)} = 0` if and only if `g` is regular (where `𝔠` denotes the Lie
algebra of `C`); on the other hand, since `C` is nilpotent, one sees at once that `ad_𝔠(g)` has only the eigenvalue
`1`, which proves that the multiplicity of the eigenvalue `1` in `ad_𝔤(g)` is `⩾ ρ`, and equal exactly to `dim C = ρ`
if and only if `g` is regular. In particular, the polynomial above is divisible by `(t − 1)^ρ`. Since the relation of
divisibility by `(t − 1)^ρ` is expressed by linear relations (with integer coefficients) among the coefficients of the
polynomial, and these relations are satisfied for `g ∈ U(k)`, `U` being a dense open, it follows (`G` being reduced)
that they hold for all `g`; in fact one has a relation
...
is the analogous decomposition of `ad(g)` (loc. cit. cor. to th. 3), so `ad(g)` and `ad(g_s)` have the same
eigenvalues (counted with multiplicity); in particular the eigenvalue `1` appears with the same multiplicity in
`ad(g)` and in `ad(g_s)`.

Moreover, by *Bible* 7 th. 2 cor. 1, `g` is regular if and only if `g_s` is. Hence to prove b), we may assume
`g = g_s`, i.e. `g` semisimple, so contained in a maximal torus by *Bible* 6 th. 5 c), and a fortiori in a Cartan
...
smooth, the Lie algebra `𝔤′` of `G′` is none other than `𝔤/𝔷`, where `𝔷 = Lie(Z)`, and `ad(g′)` is none other than
`ad(g)_{𝔤/𝔷}`; hence the multiplicity of the eigenvalue `1` in `ad(g)` equals `d = dim Z` plus the multiplicity of the
eigenvalue `1` in `ad(g′)`, whence at once the first is equal to the nilpotent rank of `G` if and only if the second
is equal to the nilpotent rank of `G′`. Thus we are reduced to the case of `G′`; now `G′` being affine by XII 6.1,
this case has already been treated. This completes the proof of 2.6.
...
necessary and sufficient that there exist exactly `d` conjugates `H_i` of `H` containing `g`, and that for each `i`,
`g` be a regular element of `H_i`; or again, that there be at most `d` conjugates of `H` containing `g`, and that `g`
be regular in one of them. If so, and if `C` is the unique Cartan subgroup of `G` containing `g`, then the conjugates
of `H` containing `g` are the conjugates of `H` containing `C`.*
...
Let us prove b). Suppose first `g` regular in `G`, let `C` be the unique Cartan subgroup of `G` containing `g`. Then
by 2.3 b) there exist exactly `d` conjugates `H_i` of `H` containing `C`. Since `(𝔤/𝔠)^{ad(g)} = 0`, i.e. `ad(g)` has
no eigenvalue `+1` on `𝔤/𝔠`, one has a fortiori `(𝔤/𝔥_i)^{ad(g)} = 0`, hence by 2.3 c) there are exactly `d`
conjugates of `H` containing `g`, namely the `H_i`. For such an `H_i`, a Cartan subgroup of `H_i` <!-- original page 268 -->
containing `g` is a Cartan subgroup of `G` containing `g` by a), hence equals `C`, which proves that `g` is regular in
`H_i`. Conversely, suppose that there are at most `d` conjugates `H_i` of `H` containing `g`, and that `g` is regular
in one of them, which we may assume to be `H`. Let us prove that `g` is regular in `G`. Since `g` is regular in `H`,
it is contained in a unique Cartan subgroup `C` of `H`; by a) this is a Cartan subgroup of `G`. Let `C′` be a Cartan
subgroup of `G` containing `g`; let us prove `C′ = C` (which will prove that `g` is regular in `G`). Indeed, by 2.3 b)
there exist exactly `d` conjugates of `H` containing `C′`, and as these latter contain `g`, they are necessarily the
`H_i`, hence the `H_i` and in particular `H` contain `C′`. Hence `C`, `C′` are two Cartan subgroups of `H` (by a)) that
contain the same regular element `g` of `H`, so they are equal. *QED*.

Let us prove c). Denoting by `ν(u)` the nullity[^N.D.E-XIII-5] of `id − u`, for an endomorphism of a
...
`G` by a)) and `⩾ 0`; hence one has `ν(ad(g)_𝔤) = ρ` if and only if `ν(ad(g)_𝔥) = ρ` and `ν(ad(g)_{𝔤/𝔥}) = 0`, i.e.
`g` is regular in `G` if and only if `g` is regular in `H` and `ad(g)_{𝔤/𝔥}` has no non-zero invariants. *QED*.
...
with `G` connected, where `H` has a connected component `H_i` such that for no `h ∈ H_i(k)` one has
`(𝔤/𝔥)^{ad(h)} = 0`, i.e. the morphism `(g, h) ↦ ad(g) · h` from `G × H_i` to `G` is étale (nor even quasi-finite) at
no point (take for example for `H` the normalizer of the maximal torus in `SL(2)_k`). Similarly, even if the image
`H″` of `H` in the finite group `G″ = G/G⁰` equals `G″`, it is not necessarily true that the union of conjugates of `H`
in `G` is dense (take for `G` the semidirect product of `ℤ/2ℤ` with `G⁰ = SL(2)_k` on which it acts by "symmetry", and
for `H` the semidirect product `ℤ/2ℤ · T`, where `T` is a maximal torus of `G⁰`). On the other hand, if one does not
assume a priori that `H⁰` contains a Cartan subgroup of `G⁰`, but that the union of conjugates of `H` in `G` is dense,
then `H⁰` necessarily contains a Cartan subgroup of `G⁰`: to verify this, one may evidently suppose `G` connected, and
it suffices to redo the proof of 2.1 (vi) ⇒ (i), which is valid without assuming `H` connected.*
...
Suppose first that we are over a base field `k`, not necessarily algebraically closed. Since conditions 2.1 (i bis),
(iv bis), (v), (vi) are invariant under extension of the base field, one sees by passing to the algebraic closure
`k̄` of `k` that they are equivalent to each other, and equivalent to the fact that `H_{k̄}` contains a Cartan subgroup
of `G_{k̄}`. When this condition is satisfied, then (with the notation of 2.3) it will still be true that `k(X)` is a
finite separable extension of `k(G)`, of degree `d` independent of any extension of the base. If `U` is the largest
open of `G` such that `ψ` induces a morphism `ψ⁻¹(U) → U` that is finite and étale, then the formation of `U` commutes
with extension of the base field. If `ψ` is birational, then `U` is also the largest open of `G` such that `ψ` induces
an isomorphism `ψ⁻¹(U) ⥲ U`, and if then `g ∈ U(k)`, there exists a subgroup `H′` of `G` and only one, conjugate to
`H` over the algebraic closure `k̄` of `k`, such that `H′ ∋ g`.
...
`K` of `k` and every point `g ∈ G(K)`, `g` is a regular point of `G_K` if and only if `g ∈ U(K)`. If `g ∈ U(k)`,[^N.D.E-XIII-6]
one sees that `g` is contained in one and only one Cartan subgroup of `G`, as we shall show under more general
conditions below.
...
Suppose this functor is representable by a smooth prescheme over `S`; we give in XV a necessary and sufficient
condition for this to be so, but we already know that this hypothesis is satisfied if `G` is affine over `S` with
locally constant reductive rank (XII 3.3), or more generally if `G` admits locally for the fpqc topology a maximal
torus (XII 7.1 a)), for example if `S` is the spectrum of a field. <!-- original page 271 --> Let `X` be the Cartan
subgroup of the `𝓒`-prescheme in groups `G_𝓒`, the "universal Cartan subgroup" of `G`. As a prescheme over `S`, `X`
therefore represents the functor
...
`g ∈ G` such that `g` is a regular element of its fiber `G_s`.*
...
topology a Cartan subgroup, and that the nilpotent rank of the fibers of `G` is locally constant. The same holds for
the dimension of the fibers of `G`, and up to localizing on `S`, one may assume that both are constant, say `ρ` and
`n`. Consider then the Killing polynomial

```text
P_G(t) = tⁿ + c₁ tⁿ⁻¹ + ⋯ + c_n ∈ A[t],   where A = Γ(G, 𝒪_G).
```

The restriction of this polynomial to the fibers `G_s` of `G`, and in particular to the fibers at the maximal points
of `S`, is divisible by `(t − 1)^ρ`, which is expressed by the fact that certain linear combinations with integer
...
reduces to the case of a base field, which we may assume algebraically closed. Then there exists a Cartan subgroup
`C` of `G`, and if `N` is its normalizer, `𝓒` is identified by the conjugacy theorem XII 7.1 a) and b) with `G/N`, and
the morphism `ψ : X → G` considered here is none other than that defined in section 2. One concludes then by 2.6 b).
The same reasoning also shows that `U` is the largest open of `G` such that `ψ` induces an isomorphism
`ψ⁻¹(U) → U`.

**Corollary 3.2.** *Under the conditions of 3.1, let `g` be a regular section of `G`, i.e. such that for every
`s ∈ S`, `g(s)` is a regular point of `G_s`. Then there exists one and only one Cartan subgroup `C` of `G` such that
`g` is a section of `C`.*
...
being dense opens of Cartan subgroups of the fibers of `G_𝓒` (namely the opens of regular points in `G`). One finds
for example the following result (which will be considerably refined in the following Exposé):

**Corollary 3.3.** *Let `G` be a smooth connected algebraic group over the field `k`, `𝓣` the scheme of maximal tori
of `G` (≃ the scheme of Cartan subgroups of `G`). Then the field of functions `k(G)` of `G` is isomorphic to the field
of functions of a smooth connected affine nilpotent algebraic group `C` over the field of functions `k(𝓣)` of `𝓣`,
namely `C =` "the generic Cartan subgroup of `G`". If `G` is affine of zero unipotent rank, i.e. if the Cartan
subgroups of `G_K` are tori, then `k(G)` is a unirational extension of `k(𝓣)`.*

<!-- label: III.XIII.3.3 -->

Of course, by *generic Cartan subgroup* of `G`, we mean (by abuse of language) the Cartan subgroup of `G_{k(𝓣)}`
generic fiber of `X` over `𝓣`. It remains only to prove the last assertion of 3.3, which is contained in the following
well-known result (due to Chevalley):

**Lemma 3.4.** *Let `k` be a field, `T` a torus over `k`, `k(T)` the field of rational functions on `T`; then `k(T)`
is a unirational extension of `k`, i.e. is contained in a pure transcendental extension of `k`.*
...
`T′ = ∏_{Spec(k′)/Spec(k)} T_{k′}/Spec(k′)` is a rational variety (because it admits a dense open isomorphic to a
dense open of <!-- original page 274 --> `∏_{Spec(k′)/Spec(k)} 𝔸ⁿ_{k′}`, which is isomorphic to affine space of
dimension `mn` over `k`, where `m = [k′ : k]`). Consider the norm homomorphism from `T′` to `T` (defined whenever `T`
is a commutative group scheme over `k`); the composite `T → T′ → T` is the `m`-th power in `T`, hence dominant, hence
`T′ → T` is dominant, which proves that `T` is unirational.
...
Finally, it follows from 3.1 that the restriction of `ψ` to the open `Y_{reg}` of regular points of `Y` induces a
closed immersion
...
(since `c₀` is regular) such that `t⁻¹ t₀ ∈ N` (normalizer of `C`), i.e. `t ∈ N`. One obtains an open and closed part
of this fiber by restricting to the `t ∈ Z(k)` such that `t ∈ C(k)`. So we have found a connected component of
`ϕ⁻¹ϕ(t₀, c₀)` isomorphic to `T` (N.B. that the preceding set-theoretic reasoning indeed gives an isomorphism of
schemes is seen by replacing points with values in `k` by points with values in an arbitrary `k`-prescheme); hence
the generic fiber of `ϕ` is of dimension `⩽ dim T`, so `Im(ϕ)` is of dimension
`⩾ dim Z × C − dim T = dim Z + dim C − dim T`; now one has
`dim Z = dim Y = dim 𝓒 + dim T = dim G − dim C + dim T`, whence finally
`dim Im(ϕ) ⩾ dim G`, so `ϕ` is dominant. *QED*.

**Remarks 3.7.** *One will note that the reasoning shows moreover that the connected component at `(t₀, c₀)` of the
fiber `ϕ⁻¹(ϕ(t₀, c₀))` is isomorphic to `T`, in particular is smooth over `k`, and has the same dimension as the
generic fiber, which implies that `ϕ` is in fact smooth <!-- original page 276 --> at `(t₀, c₀)` (which one ought to
be able to verify also by calculating the tangent map). It follows that under the conditions of 3.6 the induced
morphism `Z ×_S C_{reg} → G_{reg}` (where one has set `C_{reg} = C ∩ G_{reg}`) is a smooth morphism. One sees
similarly that the analogous morphism `Z × T_{reg} → Z` (where `T` is a maximal torus of `G`) is smooth; more
generally, for every smooth connected invariant algebraic subgroup `H` of `C` containing a regular element `c₀` of
`G(k)`, the image of `Z × H → G` is dense in that of `G × H → G`.*
...
In what follows of this section, we suppose that `k` is a field, and that `𝔤` is of finite dimension over `k`. We
shall denote by `W(𝔤)` the scheme over `k` defined by `𝔤`, whose points in the `k`-algebra `A` are the elements of
`𝔤 ⊗_k A`. If `a ∈ 𝔤`, the characteristic polynomial of `ad(a)` is also called the *characteristic polynomial* or
*Killing polynomial* of `a` in `𝔤`, namely
...
`k`-algebra, one sees that the `c_i(a)` come from well-determined sections `c_i` of the structural sheaf of `W(𝔤)`,
i.e. from elements of the symmetric algebra `A = Sym_k(𝔤^∨)`, where `𝔤^∨` is the dual of the `k`-module `𝔤`. (When `k`
is an infinite field, the `c_i` are determined by knowing the corresponding polynomial functions `𝔤 → k`, but this is
no longer the case if `k` is a finite field.) Let `r` be the largest integer such that the Killing polynomial
...
`ad_{Nil(a)}(x)` is a nilpotent endomorphism. One may assume `k` algebraically closed; then as
`ad(a)_{𝔤/Nil(a)}` is injective, there exists a non-empty open `U` of `W(Nil(a))` such that for every `x ∈ U(k)`,
`ad(x)_{𝔤/Nil(a)}` is injective, hence `Nil(x) ⊂ Nil(a)`; one may further suppose `U` contained in the open of points
where `c_{n−r}` does not vanish (since this open is non-empty by `c_{n−r}(a) ≠ 0`), and then `Nil(x)` having the same
dimension as `Nil(a)`, one will have `Nil(x) = Nil(a)`. Consequently, for every `x ∈ U(k)`, `ad(x)_{Nil(a)}` is
nilpotent, and by the principle of extension of algebraic identities, this remains true for every `x ∈ Nil(a)`, hence
`Nil(a)` is nilpotent.
...
in `𝔤`). In any case, the notion of regular element of `𝔤` is invariant under extension of the base field, and the
set of points of `W(𝔤)` that are regular (i.e. that come from regular points of `W(𝔤)` with values in a suitable
extension of `k`) is open, since identical to `W(𝔤)_{c_{n−r}}` (set of points where `c_{n−r}` is invertible).
...
The equivalence of (i) and (i bis) is trivial by 4.3, and these conditions trivially imply (ii). The equivalence of
(ii) and (ii bis) is also trivial, as is (ii bis) ⇒ (iii) (cf. 4.1). It remains to prove the implication (iii) ⇒ (ii
bis), the only one moreover that uses the fact that `k` is infinite, and which follows at once from:
...
finitely many non-zero stable subspaces `V_i` (`1 ⩽ i ⩽ n`), such that for every `i`, and every `x ∈ 𝔡`, `u(x)|V_i`
has a single eigenvalue `λ_i(x)` (cf. Bourbaki, *Groupes et Algèbres de Lie*, Chap. I, §4, Exercise 22). Let `c_i(x)`
be the constant term of the characteristic polynomial of `u(x)|V_i`, so that `λ_i(x) = 0` if and only if `c_i(x) = 0`.
Then `c_i` is a polynomial function on `𝔡`, and the hypothesis means that `𝔡` is the union of the sets of zeros of the
`c_i`. So one of the `c_i` is zero, which reduces us (replacing `V` by `V_i`) to the case where `V` is such that the
`u(x)` (`x ∈ 𝔡`) are nilpotent. But then Engel's theorem (Bourbaki loc. cit. th. 1) implies that there exists a
non-zero `v` in `V` annihilated by `𝔡`. *QED*.
...
general case (`k` not necessarily infinite) one will say that `𝔡` is a Cartan subalgebra of `𝔤` if it becomes a
Cartan subalgebra under one (and hence any) extension of the base field `k → k′`, with `k′` infinite. This thus
implies that `𝔡` is nilpotent and equal to its own normalizer.
...
containing `a` is identical to the preceding one. For b) one notes that the nullity[^N.D.E-XIII-9] of `ad(a)_𝔤` is
equal to the sum of the nullities of `ad(a)_𝔡` and of `ad(a)_{𝔤/𝔡}`, and as the first equals `r`, the sum is equal
to `r` if and only if `ad(a)_{𝔤/𝔡}` is injective. *QED*.
...
The condition is trivially necessary; let us prove that it is also sufficient. If it is satisfied, then `𝔡′ = u(𝔡_A)`
is a Lie subalgebra containing `a_A`, and every element `b` of which is such that `ad(b)_{𝔡′}` is nilpotent (because
`𝔡′` is isomorphic to `𝔡_A`, which has this property, as follows at once from the definition of "nilpotent" in
Bourbaki, *Groupes et Algèbres de Lie*, Chap. I, §4, def. 1). Taking `b = a_A`, one sees that the nil-space
`Nil(b, 𝔤_A)` contains `𝔡′`; on the other hand it is equal to `𝔡_A`, and as `𝔡′` is locally a direct factor in the
module `𝔤_A` (`𝔡` being so), hence in `𝔡_A`, and as it is a projective module of the same rank `r` as the latter, one
concludes that it equals it. *QED*.
...
infinite), and that in statements b) and c), one may assume `k` infinite, which we shall do. If `𝔥` contains the
Cartan subalgebra `𝔡 = Nil(a, 𝔤)`, then `a` is a regular element of `𝔤` such that `ad(a)_{𝔤/𝔥}` is injective, hence
(i) ⇒ (ii). Conversely, if (ii) is satisfied, then for an element `a` "sufficiently general" of `𝔥`, `a` satisfies
simultaneously the two conditions of (ii), hence `Nil(a, 𝔤)` is a Cartan subalgebra of `𝔤` and is contained in `𝔥`,
so one has (i). So (i) and (ii) are equivalent. Suppose them satisfied, let `a` be a variable element of `𝔥`; then
<!-- original page 283 -->
...
on the other hand, the two terms of the right-hand side are respectively `⩾ r₀ =` nilpotent rank of `𝔥`, and `⩾ 0`,
the equalities being moreover attained[^N.D.E-XIII-10] for an element "sufficiently general" of `𝔥`. Moreover, one has
also `rank_k Nil(a, 𝔤) ⩾ r =` nilpotent rank of `𝔤`, with equality attained for an element "sufficiently general" of
`𝔥`, and attained if and only if `a` is regular in `𝔤`. One concludes from this that one has `r = r₀`, and that `a` is
regular if and only if the two terms of the right-hand side of `(✱)` equal respectively `r₀` and `0`, i.e. if and only
if `a` is regular in `𝔥` and `ad(a)_{𝔤/𝔥}` is injective, which proves b), and c) follows trivially by taking an
element `a` in `𝔡` regular in `𝔤`, so that `Nil(a, 𝔤) = 𝔡`. Moreover, the preceding result shows that (i) ⇒ (iii);
finally (iii) ⇒ (i), because under (iii), an element sufficiently general `a` of `𝔥` is regular in `𝔥` and in `𝔤`,
hence `Nil(a, 𝔥) ⊂ Nil(a, 𝔤)` are respectively Cartan subalgebras of `𝔥` and of `𝔤`, and as they have the same rank
over `k`, they are identical, which proves (i). This completes the proof of 4.8.
...
Let `G` be a smooth algebraic group over the field `k`, and `𝔤` its Lie algebra. Let `𝔥` be a Lie subalgebra of `𝔤`.
Let `G` act on `W(𝔤)` by the adjoint representation, and consider the subscheme `W(𝔥)`. The construction of section 1
...
- *(xii) There exists a smooth algebraic subgroup `H` of `G` with Lie algebra `𝔥`, and `𝔥` contains a Cartan
  subalgebra `𝔡` of `𝔤`.*
...
On the other hand (iii) means, when `k` is algebraically closed, that there exists a `k`-rational point of `G × W(𝔥)`
at which the tangent map to `ϕ` is surjective, and one sees at once that this point may be taken of the form `(e, a)`,
where `a ∈ 𝔥` (up to transforming it by an operation of `G(k)`). One concludes that if `k` is infinite (not
necessarily algebraically closed) this condition (evidently sufficient) of generic smoothness is still necessary. Now
the tangent map is easily calculated: identifying the tangent space to `W(𝔥)` at `a` with `𝔥`, it is the map

```text
(ξ, x) ↦ [ξ, a] + x
```

from `𝔤 × 𝔥` to `𝔤`. To say that it is surjective also means that `ad(a)_{𝔤/𝔥}` is surjective, or what amounts to
the same, injective. This proves the equivalence of conditions (ii) and (iii). Moreover, (ii) evidently implies
`𝔥 = 𝔫`, and (iii) implies that `ψ` is generically smooth, <!-- original page 286 --> since if `ϕ` is smooth at a point
`u`, it follows (`q` being flat) that `ψ` is smooth at `q(u)`. So (ii), (iii) imply (v). Let us prove that they imply
(i). For this, note that since `ψ` is dominant, and the set of regular points of `𝔤` is open dense, it follows that
`𝔥` contains regular elements of `𝔤`, hence a "sufficiently general" element `b` of `𝔥` is regular in `𝔤` and satisfies
`ad(b)_{𝔤/𝔥}` injective, so `Nil(b, 𝔤) ⊂ 𝔥`, hence `𝔥` contains the Cartan subalgebra `𝔡 = Nil(b, 𝔤)`. So (i), (ii),
(iii) are equivalent; finally (i) ⇔ (iv), since we have already noted that if `𝔥` contains a Cartan subalgebra, it
has the same rank as `𝔤` (4.6), so (i) ⇒ (iv); conversely, if (iv) is satisfied, then `𝔥` contains a regular element
of `𝔤`, and since it has the same rank as `𝔤`, it contains a Cartan subalgebra by 4.6.
...
and `𝔥 = 𝔫`, hence (x); and one sees similarly (x) ⇒ (viii) by applying 5.2 b). On the other hand, (viii) implies
(ix), since it implies `𝔥 = 𝔫`, hence `𝔥` is the Lie algebra of the smooth algebraic subgroup `N` of `G`; and
conversely (ix) ⇒ (viii), since `H` normalizes its Lie algebra `𝔥`, so it is contained in `N`, and as `H` is smooth and
has the same Lie algebra as `N`, it follows that `N` is smooth.

Let us finally prove the equivalence of conditions (xi), (xii), (xiii) and the fact that they entail (iii) (which
will complete the establishment of our diagram of implications). One has (xi) ⇔ (xiii), since if `𝔫 = 𝔥`, then by 5.2
b) one has `dim X ⩾ dim G`, hence (xi) is then equivalent (given that `W(𝔤)` is normal) to the fact that `ψ` is
generically unramified, which is equivalent also to (xiii) by (1.1 (ii) ⇔ (iii)), proceeding as above for the proof
of (ii) ⇔ (iii). Since (xi) ⇒ (x) ⇒ (viii) by what we have seen, one sees that (xi) implies that `N` is smooth, i.e.
`q : G × W(𝔥) → X` is smooth, hence the composite `ϕ = ψ ∘ q` is generically smooth, i.e. one has (iii). Since (iii)
⇒ (i), it also follows that (xi) ⇒ (xii).[^N.D.E-XIII-11] Finally (xii) ⇒ (xi), since one evidently has (xii) ⇒ (i),
so as one has seen (i) ⇒ (iii) ⇒ (v), one has (xii) ⇒ (v); it follows that one has also (xii) ⇒ (ix), and as one has
seen (ix) ⇒ (x), it follows that (xii) ⇒ ((v) and (x)), hence (xii) ⇒ (xi) since generically étale = generically
smooth + generically quasi-finite.
...
values in `k` are the same; on the other hand the inclusions `H ⊂ Norm_G(H) ⊂ N` show that `Norm_G(H)` and `N` have
the same Lie algebra, hence they are identical.
...
*When one is under the equivalent conditions (xi) to (xiii), let `H` be the algebraic subgroup of `G` considered in
5.3. Then the preceding conditions are also equivalent to the following conditions:*

- *(iv) `ψ` is étale at `(ē, a)`.*
- *(v) Denoting by `M⁰_a` the connected component of `e` in the transporter `M_a` of `a` to `𝔥`, <!-- original page 289 -->
  endowed with the structure induced by `M_a`, one has*
...
flat at `(e, a)`, hence smooth since its fiber is at this point. The equivalence of (i) and (iii) has been seen in
the proof of 5.1 as resulting from the simple calculation of the tangent map. Moreover, one has seen in 4.8 b) that
"`a` regular in `𝔤`" ⇒ (iii). Under conditions (xi) to (xiii), since `q : G × W(𝔥) → X` is smooth (`N` being smooth),
it follows that (i) is equivalent to `ψ` smooth at `(ē, a)`, and as `ψ` is generically étale, this is equivalent to
(iv). Finally, as was noted at the end of the proof of 1.1, (iv) implies that `N` is the prescheme induced on `M_a`
by an open and closed part of `M_a`, whence (v); finally (v) ⇒ (ii) trivially (or again (v) ⇒ (iv) by 1.1, since `ψ`
is dominant and `W(𝔤)` normal, "unramified" is here equivalent to "étale"). This completes the proof of 5.4.

**Corollary 5.5.** *Let `G` be a smooth algebraic group over a field `k`, `H` a smooth algebraic subgroup such that
its Lie algebra `𝔥` contains (at least over a suitable extension of `k`) a Cartan subalgebra of the Lie algebra `𝔤`
of `G`. Let `K` be a connected algebraic subgroup of `G` (not necessarily smooth), with Lie algebra `𝔨`; suppose that
`𝔨` contains a regular element `a` of `𝔤` (at least over a suitable extension of `k`). Then `H` contains `K` if and
only if `𝔥` contains `𝔨`.*

<!-- label: III.XIII.5.5 -->

Indeed, by 5.4 (iii) ⇒ (v) one has `H = M⁰_a` (N.B. of course, this relation being invariant under extension of the
base field, it is valid without the hypothesis that the latter be infinite!); on the other hand `𝔨 ⊂ 𝔥` evidently
implies `K ⊂ M_a`, hence as `K` is connected, `K ⊂ M⁰_a`, whence `K ⊂ H`. *QED*.
...
subalgebra. Now by 5.1 (ii) ⇒ (i), `𝔡` contains a Cartan subalgebra `𝔡′ = Nil(a, 𝔤)`, and by 4.3 one concludes from
the fact that `𝔡` is nilpotent that `𝔡 = 𝔡′`. To prove b), one notes that `Nil(a, 𝔤)` is a Cartan subalgebra of `𝔤` by
a), hence `a` is regular.

**Corollary 5.8.** *Let `G` be a smooth algebraic group over a field `k`, `H` a smooth algebraic subgroup, `𝔤`, `𝔥`
the Lie algebras; suppose that after suitable extension of the base field, `𝔥` contains a Cartan subalgebra of `𝔤`.
Let `𝔡` be a Lie subalgebra of `𝔥`; then it is a Cartan subalgebra of `𝔥` if and only if it is a Cartan subalgebra of
`𝔤`.*
...
Given 4.8 c), it remains to show that if `𝔡` is a Cartan subalgebra of `𝔥`, it is a Cartan subalgebra of `𝔤`; for
this one is reduced to showing that `𝔡` contains an element `a` regular in `𝔤`, assuming (which is permissible) `k`
algebraically closed. But since there is a dense open in `𝔥` consisting of regular points of `𝔤`, our assertion
follows from 5.1 (i) ⇒ (vii) applied to `(𝔥, 𝔡)`.
...
*(whose fiber at `a ∈ 𝔤` has as points with values in `k` the set of Cartan subalgebras of `𝔤` containing `a`). Then
`ψ` is a birational morphism.*

*d) With the notation of c), let `U` be the largest open of `W(𝔤)` such that `ψ` induces an isomorphism* <!-- original page 292 -->
...
*Proof.* Let us apply 5.1 when `𝔥` is a Cartan subalgebra `𝔡` of `𝔤`; let us show that the strongest conditions (xi)
to (xiii) are then satisfied. This is evident, either in form (xiii) given 4.7 (which implies that for a regular
element `a` of `𝔤` contained in `𝔡`, one has `N = M_a`), or in form (xi) = (x) + (i), since condition (i) is trivial
and condition (x) follows from the fact that a regular point of `𝔤` is contained in a single Cartan subalgebra of `𝔤`
(4.6 a)), and a fortiori in a single conjugate of `𝔡`. Then b) follows from 5.3, and c) follows from the fact that
`ψ` is generically étale and that a sufficiently general point (more precisely, a regular point) of `𝔤` is contained
in a single Cartan subalgebra of `𝔤`. Under these conditions, the equivalence of conditions (i) to (v) on `a` is an
immediate consequence of Zariski's Main Theorem for the separated birational morphism `ψ`, given that `W(𝔤)` is normal
and `X` is integral. <!-- original page 293 --> The equivalence of (v) and (vi) is a particular case of 5.4 (iii) ⇔
(iv) (reducing to the case where `a ∈ 𝔡` by transforming `a` by a suitable element `g ∈ G(k)`), given 4.6 b).
Moreover, by 5.4, (v) and (vi) are also equivalent to `N ⊃ M⁰_a`, and by 4.7 already invoked, this even implies
`N = M_a`. This proves d).

Of course, b), c), and the equivalence of (i), (iv), (v), (vi), (vii) remain valid over an arbitrary base field. Let
us now prove a) using the fact that `k` is algebraically closed. By 5.1 (i) ⇒ (vii), `ψ : X → W(𝔤)` is dominant, hence
there exists a dense open `V` of `W(𝔤)` such that every `a ∈ V(k)` is the image of an element of `X(k)`, i.e. is
contained in a conjugate of `𝔡`. Applying this result to a second Cartan subalgebra `𝔡′` of `𝔤`, one sees that one
may take `V` such that every element of `V(k)` is a conjugate of an element of `𝔡` and of an element of `𝔡′`. Taking
a regular element in `V(k)`, it follows that there is a conjugate `𝔡″` of `𝔡′` which contains an element of `𝔡`
regular in `𝔤`, hence which equals `𝔡` by 4.6 a). This completes the proof of 6.1.

**Definition 6.2.** *Let `G` be a smooth algebraic group over a field `k`. One calls a* subgroup of type (C) *of `G`
any smooth connected algebraic subgroup whose Lie algebra is a Cartan subalgebra of `𝔤`. One calls the* infinitesimal
rank *of `G` the nilpotent rank of its Lie algebra `𝔤`, equal to the dimension of any subgroup of type (C) of `G`.*

<!-- label: III.XIII.6.2 -->

From 6.1 b) one concludes at once:

**Corollary 6.3.** *Under the conditions of 6.2, the map `D ↦ 𝔡 = Lie D` establishes a bijective correspondence
between subgroups of type (C) of `G`, and Cartan subalgebras `𝔡` of `𝔤`. If `D` is a subgroup of type (C) of `G`, `D`
is its own connected normalizer:*
...
the Lie subalgebras `𝔥` of `𝔤` satisfying the strongest condition (xii) of 5.1 (and by 5.5, inclusion relations
between such subgroups can already be recognized on their Lie algebras).*
...
fact, we shall see in the following Exposé a more precise result: every Cartan subgroup `C` of `G` is contained in
one and only one subgroup of type (C) of `G`.*

*c) One should not confuse in general Cartan subgroups and subgroups of type (C): the subgroups of type (C) of `G`
are Cartan subgroups <!-- original page 295 --> if and only if they are nilpotent (Cartan subgroups being indeed
maximal nilpotent subgroups), and it may happen that `𝔤` is nilpotent without `G` being so (example: `SL(2)_k` for `k`
of characteristic 2); then the Cartan subalgebras of `𝔤` are identical to `𝔤`, i.e. `G` is a subgroup of type (C) of
itself if `G` is connected, but it is not a Cartan subgroup of `G`! On the other hand, we shall see in XIV that if
`G` is a semisimple adjoint group, then its subgroups of type (C) are its Cartan subgroups. Similarly, in
characteristic 0, without restriction on the smooth algebraic group `G` over `k`, the same conclusion is valid: this
follows at once from the fact that in characteristic 0, a connected algebraic group is nilpotent if (and only if) its
Lie algebra is. This follows at once from the fact that in characteristic zero, the center `Z` of the connected
algebraic group `G` has as Lie algebra the center of `𝔤` (for one obtains a priori the space of invariants of `𝔤`
under the adjoint representation of `G`; now `G` being connected and `k` of characteristic zero, these are also the
"invariants" in the infinitesimal sense of the adjoint representation of `𝔤`, i.e. the center of `𝔤`), and that by
Cartier's theorem (cf. VI_B 1.6.1) `Z` is smooth.
