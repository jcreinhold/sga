...
XXII, 1.14 equipped with `Δ`). Let `p` be the integer defined in Exp. XXII, 4.2.2. One then has:
...
`(Z_{Δ_1}, T, M, R', Δ_1, (X_α)_{α ∈ Δ_1})` is a pinned `S`-group. We shall always equip `Z_{Δ_1}` with this pinning. In
particular, we shall consider the groups

```text
Z_α = Z_{{α}},    Z_{αβ} = Z_{{α, β}}.
```

We denote by `B_{Δ_1} = B ∩ Z_{Δ_1}`; one knows (*loc. cit.*) that this is the canonical Borel subgroup[^N.D.E-XXIII-3]
of `Z_{Δ_1}`, and that its unipotent part is `U_{Δ_1} = U ∩ Z_{Δ_1}`. In particular,
...
*be a morphism of groups and `h_α (α ∈ Δ)` elements of `H(S)`. In order that there exist a morphism of groups
(necessarily unique)*
...
*Proof.* Indeed, equip `(Sch)` with the topology `𝒯` generated by the pretopology whose covering families are the direct
sums; the hypothesis of the statement says that `H` is a `𝒯`-sheaf. Let `L` be the free group with generators
...
which shows that `f` and `g` coincide on each `U_α`. It follows that `f` and `g` coincide on `Ω`, hence coincide (Exp.
XXII, 4.1.11).
...
otherwise, one takes a `w ∈ W` such that `w^{-1}(α) ∈ Δ`; writing `w` as a product of simple
reflections,[^N.D.E-XXIII-4] one is reduced to proving that if the formula is true for `α` and for every `n`, it is also
true for `w_{α_0}(α)` and `t ∈ T(S')`, where `α_0 ∈ Δ`. Now, by 2.1 (i),
...
It suffices to give the proof for `U`. By virtue of Exp. XXII, 5.5.8, it suffices to verify that for every pair `α < β`
of positive roots, one has for all `x_α ∈ U_α(S')`, `x_β ∈ U_β(S')`, `S' → S`,
...
**Lemma 2.1.5.** *Let `n ∈ Norm_G(T)(S)` be such that `int(n) U = U^-` (i.e. `n` is the symmetry of the Weyl
group[^N.D.E-XXIII-6] (Exp. XXI, 3.6.14)). For every `u ∈ U(S')`, `S' → S` (resp. `u ∈ U^-(S')`, `S' → S`), one has*
...
This is trivial if `n ∈ T(S)` (by 2.1.3). The two sides of the preceding formula define morphisms of `Ω ∩ int(n)^{-1} Ω`
into `H`; to verify that they coincide, it suffices to verify that there exists an open `V_n` of `Ω` containing the unit
section such that `int(n) V_n ⊂ Ω`, and that `f ∘ int(n)` and `int(f_N(n)) ∘ f` coincide on `V_n`. By virtue of the
structure of `Norm_G(T)`, it suffices to prove that if the preceding assertion is true for an `n' ∈ Norm_G(T)(S)` and if
`α ∈ Δ`, it is true for `n = n' w_α`. Set
...
If all the `γ_i` are positive, i.e. belong to `R^+_{αβ}`, one may apply condition (v) of 2.3; using the notation of 2.3
(v), one concludes at once by induction that
...
Since `γ_{j+1} = s_{α_j}(γ_j)`, it follows at once that `γ_j = α_j`, by Exp. XXI, 3.3.2, hence that `γ_j` is `α` or `β`,
and one may decompose `n` as
...
For every root `α ∈ R`, there exists an `n ∈ Norm_G(T)(S)` such that `n(α) ∈ Δ`. Then define `f'_α(x)` as the expression
of 2.3.4. This is independent of the choice of `n`, and `f'_α` is indeed a morphism of groups. Property (iii) is
satisfied by construction. The first assertion of (i) is clear (take `n = e`), the second too (take `n = w_α` and apply
2.3.2); if `γ ∈ R^+_{αβ}` (`α, β ∈ Δ`), there exists `n ∈ Norm_{Z_{αβ}}(T)(S)` such that `n(t) = α` or `β`; one then
applies (iii) and conditions (5) and (6) and has proved (ii).
...
one sets `f_T = D_S(h)`, `h_α = w'_{d(α)}`. The conditions of 2.3 are satisfied (note that `q(s_α(β)) = q(β)` (Exp. XXI,
6.8.4) and that one always has `D_S(h)(t_α) = t'_{d(α)}`), and one concludes at once.
...
*If condition (a) is satisfied, one may now define for each `γ ∈ R^+_{αβ}` an `f_γ : U_γ → H` as in 2.3.5; one must then
write:*
...
*(iii) Let `X_α ∈ W(g_α)^×(S')`, `X_β ∈ W(g_β)^×(S')`, and `w ∈ Norm_G(T)(S')` such that `w(α) = β`; define
`z ∈ G_m(S')` by*
...
conditions, setting `v_α = f_α(u_α)` for `α ∈ Δ`:
...
each `S' → S` the group `H(S')` generated by `T(S')`, `U_α(S')`, `α ∈ R`, and `w_α`, `α ∈ R`, subjected to relations
analogous to (1), (2), (3₁), (4), (3₂), (A), (B); then `G` is none other than the sheaf associated with the presheaf
`S' ↦ H(S')`.
...
Now, the hypothesis that `x ↦ x^p` is an endomorphism of `G_{a, S}` (and `S ≠ ∅`) entails that `p = 1` or that `S` is of
characteristic `p`; in all cases `(-1)^q = -1` (if `q` is even, `p = 2` and `1 = -1`). Consequently,
...
The positive roots are `{α, β, α + β, 2α + β}` and `{α', β', α' + β', α' + 2β'}` (note that the "short" simple roots are
`α` and `β'`). One has
...
extending the `f_{αβ}`. This is evidently a morphism of pinned groups such that `R(f) = h`, which completes the proof of
Theorem 4.1.
...
**Corollary 5.4 ("Uniqueness of the Chevalley schemes").** *Let `G` and `G'` be two reductive groups over `ℤ` possessing
split maximal tori.*[^XXIII-5-1] *The following conditions are equivalent:*
...
connected fibers. Let `s ∈ S` be such that `G_s` and `G'_s` are isomorphic; there exists then an étale `S' → S` covering
`s` such that `G_{S'}` and `G'_{S'}` are isomorphic.*
...
If one has a morphism `S' → S` (`S'` nonempty), one may identify `Ép_{S'}(R)` with `Ép_S(R) ×_S S'`, etc. In particular,
if `Ép_{Spec(ℤ)}(R)` exists (we shall see that this is always the case), one denotes it `Ép(R)`, and one has
...
say that it is locally isomorphic (for the étale or (fpqc) topology) to `Ép_S(R)`. Likewise, by virtue of the
conjugation theorems, it comes to the same thing to say that `(G, T)` is a reductive `S`-group of type `R` equipped with
a maximal torus, or to say that it is locally isomorphic to `(Ép_S(R), T_S(R))`; likewise with Borel subgroups or
Killing couples.
...
Let us now construct a system `(X_α)_{α ∈ R}` in the following manner. For every `α ∈ R`, choose a sequence `{α_i} ⊂ Δ`
as above, and define `X_α` by
...
generated by the `s_α`, `α ∈ Δ`, is in the invariant subgroup generated by the `(s_α s_β)^{n_{αβ}}`, `(α, β) ∈ Δ × Δ`.
One is therefore reduced to the case where `s_{α_1} ⋯ s_{α_n}` is of the form
...
Indeed, since the assertion is symmetric in `α` and `β`, one may suppose `long(α) ⩽ long(β)`, and one is reduced to 6.4.
...
[^XXIII-3-1]: In the source: "since `(α^*, β) = 2`". The correct expression `(α^*, β) = -2` was used in the previous
    section; here Demazure exploits the resulting formula `s_β(t_α) = t_α t_β^{-2} = t_α` (using `t_β^2 = e`,
    which holds in `B_2`).
...
[^N.D.E-XXIII-7]: N.D.E.: We introduce here absolute constants `A, B, …, J`. These constants will be determined in the
    pages that follow; their values are `A = B = C = D = 1`, `E = 2`, `J = -1`, `F = G = H = 3`, cf.
    3.4.8.
...
[^N.D.E-XXIII-10]: N.D.E.: To avoid a notational problem later on, we have replaced `q` by `p`, so that in what follows,
    `q` (resp. `q_1`) is an arbitrary power of `p`.
...
[^N.D.E-XXIII-13]: N.D.E.: This corollary is rendered superfluous by Exp. XXV, which shows the existence of a split
    group of type `R` over `Spec(ℤ)`, hence over any base `S`. (In fact, after XXV 1.3 one finds a
    reference to the present corollary to ensure that the reductive `ℤ`-group obtained is split, but this
...
[^N.D.E-XXIII-16]: N.D.E.: On the other hand, let us point out that if `2 = 0` on `S` and `R` is of type `C_n`, then the
    set `R'` of short roots (which is a root system of type `D_n`) is not closed in `R`, but is a part of
    type (R) of `R`, symmetric (cf. XXII 5.4.2 and 5.4.10), i.e. it corresponds to a subgroup `H_{R'}` of
    type (R) of `G` with reductive fibers: this is in particular the case for the natural embedding (in
    characteristic 2) of `SO(2n)` into `Sp(2n)`. Likewise, if `2 = 0` on `S` and `R` is of type `F_4`
    (resp. if `3 = 0` on `S` and `R` is of type `G_2`), the set `R'` of short roots (which is a root
    system of type `D_4` (resp. `A_2`)) is not closed in `R`, but corresponds to a subgroup `H_{R'}` of
    type (R) of `G`, with reductive fibers.
