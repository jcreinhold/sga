...
induction on $n$ that the condition $(\ast_{n})$ is equivalent to the following equality (cf. EGA IV₄, 16.8.8.2):
...
hence $\mathcal{A}(G)$ is an `O_S`-module of finite presentation (cf. EGA IV₁, 1.4.7). Let $\mathcal{I}$ be the
...
$G^{(p^{mn})}$ is smooth over $k$ for $m$ large, so $G$ is also, by (fpqc) descent (cf. EGA IV₄, 17.7.1).
...
$gr_{I}(A)$, hence also of the completion `Â` of $A$, hence $n_{G}$ is étale at the origin (cf. EGA IV₄, 17.6.3).
...
it follows from EGA IV₂, 2.5.2, that $G \to H \ G$ is locally free of rank $m$. Denoting $r = rg_{k}(H \ G)$, one
