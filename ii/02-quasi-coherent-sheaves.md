...
By
([EGA III 1.4.17](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#14-application-to-the-cohomology-of-arbitrary-preschemes)),
in order that the $\mathcal{H}^{i}_{Z}(F)$ be quasi-coherent it therefore suffices that the $\mathcal{H}^{i}_{U}(F)$ and
the $\mathcal{H}^{i}_{V}(F)$ be so. We may therefore assume that $Z$ is open and that the canonical immersion
$j: Z \to X$ is quasi-compact.

Since $Z$ is open, we have (I 2.2)[^N.D.E-II-1] a canonical isomorphism

$$ \mathcal{H}^{i}_{Z}(F) \simeq R^{i} j_{*}(F|Z), $$

but $j$ is separated ([EGA I 5.5.1](https://jcreinhold.github.io/ega/i/01-05-reduced-preschemes-and-separation.html#55-separation-criteria)) and quasi-compact, hence ([EGA III 1.4.10](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#14-application-to-the-cohomology-of-arbitrary-preschemes)) the $R^{i} j_{*}(F|Z) =
...
([EGA I 1.1.3.7](https://jcreinhold.github.io/ega/i/01-01-affine-schemes.html#11-the-prime-spectrum-of-a-ring)); (4.2)
and (4.3) follow from the cohomology exact sequence (I 2.7)[^N.D.E-II-1] and from the fact that $H^{i}(X, F) = 0$ for
$i > 0$, since $X$ is affine.
...
notation of
([EGA III § 1](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#1-cohomology-of-affine-schemes)),[^N.D.E-II-3]
one has:
...
([EGA III 1.4.3.2](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#14-application-to-the-cohomology-of-arbitrary-preschemes));
if $i \geqslant 2$, one uses (4.3) and
([EGA III 1.4.3.1](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#14-application-to-the-cohomology-of-arbitrary-preschemes)).
This gives us isomorphisms functorial in $M$. One verifies that, up to a sign depending only on $i$, they are compatible
with the boundary operator, whence the existence of the isomorphism of $\partial$-functors (5.1).
...
We shall now prove 6.a); this is a local question on $X$. By Corollary 4 and
([EGA I 1.3.9](https://jcreinhold.github.io/ega/i/01-01-affine-schemes.html#13-sheaf-associated-to-a-module) and 1.3.12)
we may assume that $X$ is the spectrum of a ring $A$. It therefore suffices to show that, under the hypotheses of
...
([EGA III 1.1.4.1](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#11-review-of-the-exterior-algebra-complex))
an exact sequence
...
[^N.D.E-II-3]: *N.D.E.* Recall that $H_{\bullet}(f, M)$ is the Koszul cohomology
    ...
    ([EGA III 1.1.2](https://jcreinhold.github.io/ega/iii/08-ch3-01-cohomology-affine-schemes.html#11-review-of-the-exterior-algebra-complex))
    with values in $M$, and that $H_{\bullet}((f), M)$ is the limit (loc. cit., 1.1.6.5)
    $\lim\to_{n} H_{\bullet}(f^{n}, M)$, the transition morphisms being induced by the natural morphisms
