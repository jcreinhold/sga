...
We denote by `j` (resp. `i`) the homomorphism from `X̂` into `X` (resp. from `X̂′` into `X′`). It is known that `i` and
`j` are flat.

Let `I′` be an ideal of definition of `Y′`, and let `J = f*(I′)·𝒪_X`; this is an ideal of definition of `Y`. One
therefore has:
...
Let `F` be a coherent `𝒪_X`-Module. For every `k ∈ ℕ`, we set:
...
defined for every `i ∈ ℤ` and every `k ∈ ℕ`, which gives, by passing to the associated sheaf, the graded `S`-Module
structure on `H^i`.
...
(0) `r_n` and `r_{n−1}` are isomorphisms, and `R^i f̂_*(F̂)` is coherent for `i = n − 1`;

(1) for `i = n − 1`, `ρ_i`, `ϕ_i`, and `ψ_i` are topological isomorphisms (in particular, the filtration defined on
...
Let `I` be the ideal of `A` such that `Ĩ = I′`, and let
...
We work on `ϕ_i` and `ψ_i`, which correspond to homomorphisms of modules:
...
which shows that the graded `S`-module `⨁_{k ∈ ℕ} R^i_k` is a quotient of the graded `S`-submodule
...
in which `s_i` is an isomorphism; indeed, the filtration of `H^i(X, F)` is `I`-good. Moreover, `t_i` is a monomorphism;
indeed, the functor `lim` is left exact, and, for every `k ⩾ 0`, the natural morphism `M^i / R^i_k → H^i_k` is a
monomorphism, by definition of `R^i_k`.
...
since `Q^i_k` is the image of `H^i_k`.

(b) We assume only that `H^{i+1}` is of finite type, and we focus on `t_i` (forgetting `s_i`). Since `S` is noetherian,
`Q^i` is of finite type; since `I^{k+1} Q^i_k` vanishes, we find that there exist an integer `r ⩾ 0` and an integer
`k₀ ⩾ 0` such that
...
indeed, by (b), the projective system `(H^{i−1}_k)_{k ∈ ℕ}` satisfies the Mittag-Leffler condition.
...
Suppose that `I′` is generated by a section `t′` of `𝒪_{X′}`, and denote by `t` the corresponding section of `𝒪_X`. Let
`F` be a coherent `𝒪_X`-module and let `n` be an integer. Suppose that:
...
Indeed, one observes that multiplication by `t^k` defines an isomorphism `F ⥲ J^k F`, and one deduces that
...
Suppose that `X′ = Spec(A)`, where `A` is a noetherian ring separated and complete for the `I`-adic topology. Suppose
that the `S`-module `H^i` is of finite type for `i = n − 1` and `i = n` (cf. 1.14 and 1.15). Then the hypotheses of
Theorem 1.1 are satisfied, and one finds a commutative diagram of isomorphisms:
...
Let `A` be a noetherian ring. Let `t ∈ A` and suppose that `A` is separated and complete for the `(tA)`-adic topology.
Set:
...
Let `F` be a coherent `𝒪_X`-Module. Finally, let
...
By virtue of 1.3 and 1.2, it suffices to prove that `R^i f_*(F)` is coherent for `i ⩽ n`, which follows from the
finiteness theorem 2.1.[^N.D.E-IX-4]
...
Let `f: X → X′` be an adic morphism[^IX-2-star] of formal preschemes, with `X′` noetherian. Let `I′` be an ideal of
...
Let `F` be a coherent `𝒪_X`-Module. For every `k ∈ ℕ`, the `𝒪_{X_k}`-Modules
...
(ii) The homomorphism `ψ_n` (2.3) is a topological isomorphism. The natural filtration of the right-hand side of (2.3)
is `I′`-good.
...
where `J = t𝒪_{X̂}` is an ideal of definition of `X̂`. Suppose that `A` is a quotient of a regular noetherian ring and
that:
...
Then there exists a coherent `𝒪_X`-Module `F̃` such that `F̃^∧ ≃ F`.
...
It suffices to prove that `f̂_*(F)` is a coherent `𝒪_{X̂′}`-Module, where `f̂: X̂ → X̂′` is the morphism of formal
preschemes deduced from the injection of `X` into `X′` by completion with respect to `t`. Indeed, `A` is separated and
complete for the `t`-adic topology, so there will exist an `A`-module `F′` whose completion will be isomorphic to
`f̂_*(F)`. Since `X` is an open of `X′`, one will be able to take `F̃ = F̃′ | X`.

It remains to show that 2.1 is applicable to the morphism of formal preschemes `f̂` and to `F`. Now, by hypothesis (1),
for every `k ∈ ℕ` one has an isomorphism:
...
Let `A` be a noetherian local ring and let `t ∈ r(A)` be an element of the radical of `A`. Suppose that `A` is separated
and complete for the `t`-adic topology and, moreover, a quotient of a regular ring (for example, suppose that `A` is a
complete noetherian local ring). Set
...
and take up the notation (1.31), (1.32), and (1.33). Let `F` be an `𝒪_{X̂}`-Module. Suppose that:

(1) `t` is `F`-regular,

(2) `prof_{T′} F₀ ⩾ 2`, with `F₀ = F/J F` and `J = t𝒪_{X̂}`.
