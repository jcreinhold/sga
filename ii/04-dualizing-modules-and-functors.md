...
Therefore $Y = \varprojlim X_{k}$ is an `Â`-module of finite type, as follows from
[EGA 0_I 7.2.9](https://jcreinhold.github.io/ega/i/00-07-adic-rings.html#72-adic-rings-and-projective-limits). We note
in this connection that $\mathcal{D}_{A}$ is also the category of `Â`-modules of finite type, or, if one prefers, that
