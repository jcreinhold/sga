...
**1.1.** Let `F•` and `G•` be two graded modules; then one writes
...
for the graded module of homomorphisms of graded modules from `F•` into `G•`. Thus one has
...
If `G•` is injective in each degree, then

```text
F• ↦ H•(F•, G•)
```

is an exact ∂-functor. Likewise, for arbitrary `F•`,

```text
G• ↦ H•(F•, G•)
```

is an exact δ-functor on the category of complexes `G•` that are injective in each degree.
...
Indeed, let `i: M → R(M)` be the canonical augmentation, and let `h ∈ Homˢ(R(M), R(N))`; one writes `tₛ` for the map
...
is a coboundary in `Hom•(R(N), R(P))`.
...
(iii) `Hⁿ(M) ≠ 0`, and if `A` is complete, `D(Hⁿ(M))` is of dimension `n` and
...
Let now `f: B → A` be a surjective homomorphism of local rings. Let
...
(b) for every `𝔭 ∈ X = Spec(A)` such that `dim A_𝔭 < j`, one has `Extʲ(M, A)_𝔭 = 0`;
...
hence `y ∉ Ass(H)`. If `y = yᵢ`, then `Yᵢ ⊂ Supp(M(y))`, so `T(M(y)) ≠ 0`, whence

```text
Ass(T(M(y))) = Supp(M(y)) ∩ Ass(H) ≠ ∅.
```

By the first part of the proof, this implies `y ∈ Ass(H)`, whence the lemma. QED
...
a) `Hⁱ(M)` is of finite length;

b) `∀ x ∈ X − {𝔪}, Hⁱ⁻ᵈⁱᵐ {x}_{𝔪_x}(M_x) = 0`.
...
In order that `Hⁱ(M)` be of finite length for `i ⩽ n`, it is necessary and sufficient that
